...
#define OPEN_CIRCUIT_VOLTAGE 0.04  // Voltage near zero indicates an open circuit
...
int CS_dutyCycle = 4095;       // Cold fan Duty cycle
int TEC_dutyCycle = 2000;
...
void PI_ALGO() {
  ...
  if (temperature <= 8.1 && PI_STATE == 1) {
    ...
    HS_dutyCycle = map_algo(temperature, 2, MAX_TEMP, 0, 4095);  // Scale 12-bit resolution (0-4095)
    TEC_dutyCycle = map_algo(temperature, referenceTemp, MAX_TEMP, 0, 4095);  // Scale 12-bit resolution (0-4095)
    if (HS_dutyCycle > 4095) {
      HS_dutyCycle = 4095;  // Cap the duty cycle at 100%
    }
    ...
    HS_dutyCycle = map(temperature, 2, MAX_TEMP, 0, 4095);  // Scale 12-bit resolution (0-4095)
    TEC_dutyCycle = 0;
  } else {
    if (temperature >= 8.1) {
      PI_STATE = 1;
...
void indication() {
  if (soc >= 80) {
    indici = 1;
  } else if (soc >= 55 && soc < 80) {
    indici = 2;
  } else if (soc >= 30 && soc < 55) {
    indici = 3;
